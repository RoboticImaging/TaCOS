    ...
    with conn:
        ...
        for generation in range(num_generations):
            ...
            for i in range(sol_per_pop):
                ...
                # obtain the handle of the window you want to set the focus to
                hwnd = win32gui.FindWindow(None, "ProceduralGeneration - Unreal Editor")
                ...
                # obtain the handle of the window you want to set the focus to
                hwnd = win32gui.FindWindow(None, "LearnGeometric – GeneticAlgorithmMain.py")
                shell = win32com.client.Dispatch("WScript.Shell")
                shell.SendKeys('%')
            ...
            fitness_array.append(fitness[best_match_idx[0]])
            accuracy_array.append(accuracy[best_match_idx[0]])
            inlier_num_array.append(inlier_num[best_match_idx[0]])
            inlier_ratio_array.append(inlier_ratio[best_match_idx[0]])
            mAP_array.append(mAP[best_match_idx[0]])
        ...
        iterations = list(range(1, num_generations+1))

        # Initialise the subplot function using number of rows and columns
        figure, axis = plt.subplots(3, 2)

        axis[0, 0].plot(iterations, fitness_array)
        axis[0, 0].set_title("Objective Function")
        axis[0, 0].grid(True)
        axis[0, 1].plot(iterations, accuracy_array)
        axis[0, 1].set_title("Threshold Detection")
        axis[0, 1].grid(True)
        axis[1, 0].plot(iterations, inlier_num_array)
        axis[1, 0].set_title("Inlier ORB Feature Number")
        axis[1, 0].grid(True)
        axis[1, 1].plot(iterations, inlier_ratio_array)
        axis[1, 1].set_title("Inlier ORB Featrue Ratio")
        axis[1, 1].grid(True)
        axis[2, 0].plot(iterations, mAP_array)
        axis[2, 0].set_title("OD mAP")
        axis[2, 0].grid(True)
